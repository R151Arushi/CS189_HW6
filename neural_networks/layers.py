...
class FullyConnected(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        val = np.matmul(X, self.parameters["W"])

        # Add the bias term 
        val = val + self.parameters["b"]

        out = self.activation(val)
        
        # store information necessary for backprop in `self.cache`
        self.cache = {"X" : X, "Z" : val}
        ### END YOUR CODE ##
        return out
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        self.gradients["b"] = np.matmul(dLdZ.T, np.ones((X.shape[0], 1))).reshape(self.n_out,)
...
class Conv2D(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        #calculate 
        r_vals = in_rows + 2*self.pad[0] - kernel_height
        rows_fin = int(r_vals/ self.stride + 1)

        c_vals = in_cols + 2*self.pad[1] - kernel_height
        cols_fin = int(c_vals / self.stride + 1)

        Z = np.empty((n_examples, rows_fin, cols_fin, out_channels))
        Z = Z.astype(X.dtype)

        pad_loop = np.pad(X, pad_width=((0, 0), (self.pad[0], self.pad[0]), (self.pad[1], self.pad[1]), (0, 0)))

        loop_par = W.shape[-1]
        print(loop_par)
        for r in range(rows_fin):
            for c in range(cols_fin):
                range_update = pad_loop[:, r*self.stride : r*self.stride+ kernel_height, c*self.stride : c*self.stride+kernel_width, :]

                for i in range(loop_par):
                    Z[:, r, c, i] = np.sum(range_update * W [:, :, :, i], axis=(1,2,3)) + b[0, i]
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        kernel_height, kernel_width, in_channels, out_channels = W.shape
        _, out_rows, out_cols, out_channels = dLdY.shape
        dX = np.zeros_like(X)
        dW = np.zeros_like(W)
        db = np.zeros_like(b)

        # Compute the derivative of the loss with respect to the input of this layer
        for i in range(out_rows):
            for j in range(out_cols):
                # Extract input patch
                X_patch = X[:, i*self.stride:i*self.stride+kernel_height, j*self.stride:j*self.stride+kernel_width, :]

                # Compute dLdY * df(z) where z = np.sum(X_patch * W) + b
                dLdZ = self.activation_function.gradient(np.sum(X_patch * W, axis=(1, 2, 3)) + b) * dLdY[:, i, j, :]

                # Compute the gradient of the loss with respect to the input feature maps
                dX[:, i*self.stride:i*self.stride+kernel_height, j*self.stride:j*self.stride+kernel_width, :] += np.tensordot(dLdZ, W, axes=(1, 0))

                # Compute the gradient of the loss with respect to the filter weights
                dW += np.tensordot(X_patch, dLdZ, axes=(0, 0))

                # Compute the gradient of the loss with respect to the bias terms
        ...
        self.parameters["W"] -= self.learning_rate * dW
        self.parameters["b"] -= self.learning_rate * db
